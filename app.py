...
def load_stock_from_pastebin():
    """Descarga el stock del Pastebin y lo guarda en local."""
    try:
...
def update_pastebin():
    """Actualiza el stock en el mismo Pastebin."""
    try:
...
# Carga el stock al arrancar si no existe
...
@app.get("/stock")
async def get_stock():
    """Devuelve la cantidad de cuentas disponibles."""
...
@app.post("/gen")
async def gen():
    """Devuelve una cuenta y la elimina del stock."""
    if not os.path.exists(STOCK_FILE):
        load_stock_from_pastebin()
